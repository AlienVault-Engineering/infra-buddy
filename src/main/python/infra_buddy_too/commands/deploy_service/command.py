...
def do_command(deploy_ctx, dry_run):
    ...
    for deploy in plan:
        ...
        print_utility.progress(f"Starting Deployment: {str(deploy)}")
        deploy.do_deploy(dry_run)
        print_utility.progress(
            f"Finished Deployment in {print_utility.print_time_delta(datetime.timedelta(milliseconds=(current_milli_time() - start)))}")
