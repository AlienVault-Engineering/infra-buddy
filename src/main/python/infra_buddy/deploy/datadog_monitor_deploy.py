...
class DataDogMonitorDeploy(Deploy):
    ...
    def _internal_deploy(self, dry_run):
        ...
        for monitor in to_deploy:
            ...
            if not dry_run:
                self.init_dd()
                existing_id = self.find_monitor_if_exists(monitor)
                if not existing_id:
                    response = dd.api.Monitor.create(**monitor)
                    print_utility.info("Created monitor - {}".format(response['name']))
                else:
    ...
    def init_dd(self):
        ...
        if api_key is None or app_key is None:
            print_utility.error("Can not deploy datadog monitor without configuring DATADOG_KEY and DATADOG_APP_KEY", raise_exception=True)
    ...
    def find_monitor_if_exists(self, monitor):
        name_ = monitor['name']
