...
class TestCloudFormationBuddy(object):
    ...
    def get_export_value(self, param):
        return "{bucket_name}/{root_path}".format(
            root_path=self.ctx.cf_deploy_resource_path,
...
class S3TestCase(ParentTestCase):
    ...
    def test_file_operations(self):
        ...
        try:
            ...
            self.assertEqual(
                s3_buddy._get_s3_object('sample_changeset.json')['ContentType'],
                'application/json',
                "Did not persist correct content type"
            )
        finally:
            self.clean_s3(s3_buddy)
    ...
    def test_zip_download(self):
        ...
        s3_url = "s3://{bucket}/{key}".format(
            ...
            key=s3_buddy._get_upload_bucket_key_name(file=None, key_name="test_compress.json.zip")
        )
    ...
    def test_s3_deploy(self):
        ...
        try:
            ...
            s3d = S3Deploy(
                artifact_id="test_compress.json", location=s3_url[:s3_url.rfind("/")], ctx=self.test_deploy_ctx
            )
            s3d.cloud_formation_buddy = TestCloudFormationBuddy(self.test_deploy_ctx)
            s3d.do_deploy()
            self.assertEqual(
                s3_buddy.get_file_as_string('test_compress.json'),
                "test-compress",
                "Failed to get file as string"
            )
        finally:
            self.clean_s3(s3_buddy)
